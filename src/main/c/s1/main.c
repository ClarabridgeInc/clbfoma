...
FEXPORT void
io_free(struct io_buf_handle *iobh) {
    if (iobh->io_buf != NULL) {
        xxfree(iobh->io_buf);
        (iobh->io_buf) = NULL;
    }
...
size_t
io_text_file_to_buf(struct io_buf_handle *iobh, char *filename) {
    size_t size;
    //gzFile FILE;
...
FEXPORT struct io_buf_handle *
io_init_text_file(char *filename) {
    struct io_buf_handle *iobh;

    iobh = io_init();
    if (io_text_file_to_buf(iobh, filename) == 0) {
        io_free(iobh);
        return NULL;
    }
    return iobh;
...
FEXPORT struct io_buf_handle *
io_init_buf_ptr(size_t size, char *ptr) {
    struct io_buf_handle *iobh;
...
explode_line(char *buf, int *values) {
    int i, j, items;
    j = i = items = 0;
    for (;;) {
...
int
sigma_add_number(struct sigma *sigma, char *symbol, int number) {
...
static int
io_gets(struct io_buf_handle *iobh, char *target) {
    int i;
...
struct fsm *
io_net_read(struct io_buf_handle *iobh, char **net_name) {
...
struct fsm *
fsm_read_binary_multiple(struct io_buf_handle *iobh) {
    char *net_name;
    struct fsm *net;
...
FEXPORT struct fsm *
iface_load_buf(struct io_buf_handle *iobh) {
    struct fsm *net;
    net = fsm_read_binary_multiple(iobh);
    if (net != NULL) {
        fsm_count(net);
...
void
apply_create_statemap(struct apply_handle *h, struct fsm *net) {
    int i;
    struct fsm_state *fsm;
    fsm = net->states;
...
void
apply_stack_clear (struct apply_handle *h) {
    h->apply_stack_ptr = 0;
}

void
apply_stack_pop (struct apply_handle *h) {
...
int
apply_stack_isempty(struct apply_handle *h) {
...
static void
apply_stack_push (struct apply_handle *h, int vmark, char *sflagname, char *sflagvalue, int sflagneg) {
    struct searchstack *ss;
...
int
apply_append(struct apply_handle *h, int cptr, int sym) {
...
int
apply_match_length(struct apply_handle *h, int symbol) {
    if (symbol == EPSILON) {
	return 0;
    }
...
int
apply_check_flag(struct apply_handle *h, int type, char *name, char *value) {
...
int
apply_match_str(struct apply_handle *h, int symbol, int position) {
    if (((h->mode) & ENUMERATE) == ENUMERATE) {
	if (h->has_flags && (h->flag_lookup+symbol)->type) {
	    if (!h->obey_flags) {
		return 0;
...
int
apply_binarysearch(struct apply_handle *h) {
...
void
apply_set_iptr(struct apply_handle *h) {
...
int
apply_follow_next_arc(struct apply_handle *h) {
    char *fname, *fvalue;
...
char *
apply_return_string(struct apply_handle *h) {
...
static void
apply_force_clear_stack(struct apply_handle *h) {
...
void
apply_clear_flags(struct apply_handle *h) {
    struct flag_list *flist;
    for (flist = h->flag_list; flist != NULL; flist = flist->next) {
	flist->value = NULL;
	flist->neg = 0;
...
void
apply_mark_state(struct apply_handle *h) {
...
void
apply_skip_this_arc(struct apply_handle *h) {
...
int
apply_at_last_arc(struct apply_handle *h) {
    int seeksym, nextsym;
    if (h->state_has_index) {
...
int
utf8iscombining(unsigned char *s) {
    if (*s == '\0' || *(s+1) == '\0')
	return 0;
...
void
apply_add_flag(struct apply_handle *h, char *name) {
...
void
apply_mark_flagstates(struct apply_handle *h) {
    int i;
    struct fsm_state *fsm;
...
void
apply_add_sigma_trie(struct apply_handle *h, int number, char *symbol, int len) {
...
void
apply_create_sigarray(struct apply_handle *h, struct fsm *net) {
    struct sigma *sig;
    int i, maxsigma;
    char *flag_name;
    ...
    if (h->sigma_size > 0) {
        h->sigs = xxmalloc(sizeof(struct sigs)*(h->sigma_size));
    }
    h->has_flags = 0;
    h->flag_list = NULL;
	...
	if (flag_check(sig->symbol)) {
	    h->has_flags = 1;
            flag_name = flag_get_name(sig->symbol);
	    apply_add_flag(h, flag_name);
	}
...
FEXPORT struct apply_handle *
apply_init(struct fsm *net) {
...
void
apply_clear_index_list(struct apply_handle *h, struct apply_state_index **index) {
    int i, j, statecount;
...
void
apply_clear_index(struct apply_handle *h) {
    if (h->index_in) {
...
FEXPORT void
apply_clear(struct apply_handle *h) {
    struct flag_list *flist;
	...
	for (i=0; i < h->sigma_size; ++i) {
	    if ((h->flag_lookup+i)->name != NULL) {
                xxfree((h->flag_lookup+i)->name);
            }
	    if ((h->flag_lookup+i)->value != NULL) {
                xxfree((h->flag_lookup+i)->value);
            }
	}
    ...
    while (h->flag_list != NULL) {
        xxfree(h->flag_list->name);
	flist = h->flag_list->next;
        xxfree(h->flag_list);
        h->flag_list = flist;
    }
    xxfree(h->space_symbol);
...
void
apply_set_print_space(struct apply_handle *h, int value) {
    h->print_space = value;
    h->space_symbol = xxstrdup(" ");
}

void
apply_set_print_pairs(struct apply_handle *h, int value) {
    h->print_pairs = value;
}

void
apply_set_show_flags(struct apply_handle *h, int value) {
    h->show_flags = value;
}

void
apply_set_obey_flags(struct apply_handle *h, int value) {
    h->obey_flags = value;
}

void
iface_apply_set_params(struct apply_handle *h) {
    // !!!clb!!! was global
    int g_print_space = 0; 
    int g_print_pairs = 0; 
    int g_show_flags = 0; 
...
char *
apply_net(struct apply_handle *h) {
...
void
apply_create_sigmatch(struct apply_handle *h) {
    char *symbol;
    struct sigma_trie *st;
...
char *
apply_updown(struct apply_handle *h, char *word) {
...
FEXPORT char *
apply_up(struct apply_handle *h, char *word) {
...
void
iface_apply_up(char *word, struct fsm *fsm, struct apply_handle *ah) {
    int g_list_limit = 100;
    int i;
    char *result;

    result = apply_up(ah, word); // returns always h->outstring or NULL
...
main(void) {
    ...
    iobh = io_init_text_file("bin/morphind");
    if (iobh == NULL) {
	fprintf(stderr, "%s: ", "bin/morphind");
        perror("File error");
        return 1;
    }

    p_fsm = iface_load_buf(iobh);
    if (p_fsm == NULL) {
	fprintf(stderr, "Load buf error");
        perror("Load buf error");
        io_free(iobh);
        return 2;
    }

    p_ah = apply_init(p_fsm);
    iface_apply_set_params(p_ah); // optional

    iface_apply_up("kirim", p_fsm, p_ah);
    iface_apply_up("aku", p_fsm, p_ah);
